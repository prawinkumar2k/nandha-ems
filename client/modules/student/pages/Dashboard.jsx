...
export default function StudentDashboard() {
  ...
  const { data: dashboard, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get("/api/reports/student").then(res => res.data),
  });
  ...
  const stats = [
    ...
    { label: "Pending Tasks", value: dashboard?.pendingTasks || 0, icon: <Clock className="w-5 h-5" />, color: "bg-orange-500/10 text-orange-500" },
  ];
    ...
    <MainLayout navItems={NAV} title="Student Dashboard">
      ...
      >
        ...
        <div className="w-full relative mb-8">
          <div className="h-40 w-full rounded-t-[32px] overflow-hidden bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 opacity-90">
           ...
           <motion.div variants={itemVariants} className="flex flex-col gap-6">
             <Card className="rounded-[32px] glass border-white/5 p-6 flex-1 flex flex-col hover:border-white/10 transition-colors">
                ...
                <div className="mt-auto flex items-end justify-between">
                   <div>
                     ...
                     <p className="text-3xl font-black text-orange-500">{dashboard?.skills?.examStats?.violations || 0}</p>
                   </div>
                   <div className="text-right">
                     <p className="text-4xl font-black text-primary">{dashboard?.skills?.examStats?.integrityScore ?? 100}%</p>
             ...
             <Card className="rounded-[32px] glass border-white/5 p-6 flex-1 flex flex-col hover:border-white/10 transition-colors">
                ...
                <div className="mt-auto flex justify-between items-end">
                   <div>
                     <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-1">Total Score</p>
                     <p className="text-4xl font-black">{dashboard?.skills?.overallScore || 0}</p>
                   </div>
                   <div className="text-right">
                     <p className="text-2xl font-black text-primary">LVL {dashboard?.skills?.proficiencyLevel || 1}</p>
                   </div>
                </div>
             </Card>
           </motion.div>
             ...
             <Card className="rounded-[32px] glass border-white/5 p-6 flex-1 flex flex-col hover:border-white/10 transition-colors">
                ...
                <div className="mt-auto grid grid-cols-3 gap-2 text-center">
                   <div className="bg-white/5 rounded-xl p-3">
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mt-1">Taken</p>
                   </div>
           ...
           <motion.div variants={itemVariants} className="lg:col-span-2">
             <Card className="rounded-[32px] glass border-white/5 h-full">
               ...
               <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                 {isLoading ? (
                   [1, 2, 3, 4].map(i => <Skeleton key={i} className="h-16 w-full rounded-2xl opacity-10" />)
                 ) : dashboard?.courses?.length > 0 ? (
                   dashboard.courses.map((c, i) => (
                     <div key={i} className="space-y-2 p-4 rounded-2xl bg-white/5 border border-white/5">
                       <div className="flex justify-between items-end">
                         <div>
                           <p className="text-sm font-bold tracking-tight line-clamp-1">{c.name}</p>
                         </div>
                   ...
                   <div className="col-span-2 text-center py-12 opacity-30">
                     <p className="font-bold text-sm uppercase">No Enrolled Courses</p>
                   </div>
                 )}
               </CardContent>
             </Card>
           ...
           <motion.div variants={itemVariants}>
             <Card className="rounded-[32px] glass border-white/5 p-6 flex flex-col h-full hover:border-white/10 transition-colors">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 mt-auto">
                   <div>
                     <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-1">Attended</p>
                     <p className="text-2xl font-black">{dashboard?.skills?.coding?.attended || 0}</p>
                   </div>
                   <div>
                     <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mb-1">Solved</p>
                     <p className="text-2xl font-black">{dashboard?.skills?.coding?.solvedCorrectly || 0}</p>
                   </div>
                   <div>
                     <p className="text-[10px] text-primary uppercase font-bold tracking-widest mb-1">Score</p>
